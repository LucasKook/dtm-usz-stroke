...
run_experiment <- function(mod = c("cs", "ci", "cswox", "mcc"), fml = fm, B = 50,
  ...
  for (run in 1:B) {
    ...
    if (mod != "mcc") {
      ...
      llb[run] <- evaluate(m, x = train_inp, y = Y_train, batch_size = bs)[["loss"]]
      vllb[run] <- evaluate(m, x = valid_inp, y = Y_valid, batch_size = bs)[["loss"]]
      tllb[run] <- evaluate(m, x = test_inp, y = Y_test, batch_size = bs)[["loss"]]
      ...
      if (warmstart) {
        ...
        lla[run] <- evaluate(m, x = train_inp, y = Y_train, batch_size = bs)[["loss"]]
        vlla[run] <- evaluate(m, x = valid_inp, y = Y_valid, batch_size = bs)[["loss"]]
        tlla[run] <- evaluate(m, x = test_inp, y = Y_test, batch_size = bs)[["loss"]]
      }
    } else {
    ...
    if (mod == "cs") {
      lls[run] <- evaluate(m, x = train_inp, y = Y_train, batch_size = bs)[["loss"]]
      vlls[run] <- evaluate(m, x = valid_inp, y = Y_valid, batch_size = bs)[["loss"]]
      tlls[run] <- evaluate(m, x = test_inp, y = Y_test, batch_size = bs)[["loss"]]
      ...
      train_dat$eta <- c(predict(mim, x = img_train, batch_size = bs))
      valid_dat$eta <- c(predict(mim, x = img_valid, batch_size = bs))
      test_dat$eta <- c(predict(mim, x = img_test, batch_size = bs))
      ...
      tmets <- evaluate(m, x = test_inp, y = Y_test, batch_size = nrow(Y_test))
      test_out <- data.frame(metric = names(tmets), value = tmets, data = "test",
                             epoch = nep, lr = lr, bs = bs)

      write.csv(test_out, file.path(oup, paste0("test-metrics", run, ".csv")),
                quote = FALSE, row.names = FALSE)
