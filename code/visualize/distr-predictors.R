...
dat_xy <-
  dat_xy[rowSums(is.na(dat_xy)) != ncol(dat_xy) - 1,] # no NA anymore
dat_xy_raw$mrs3 <- dat_xy$mrs3

# factor vars
dat_xy$sexm <-
  with(dat_xy, factor(ifelse(sexm == max(sexm), "m", "f")))
dat_xy$stroke_beforey <-
  with(dat_xy, factor(ifelse(
    stroke_beforey == max(stroke_beforey), "yes", "no"
  )))
dat_xy$tia_beforey <-
  with(dat_xy, factor(ifelse(
    tia_beforey == max(tia_beforey), "yes", "no"
  )))
dat_xy$ich_beforey <-
  with(dat_xy, factor(ifelse(
    ich_beforey == max(ich_beforey), "yes", "no"
  )))
dat_xy$rf_hypertoniay <-
  with(dat_xy, factor(ifelse(
    rf_hypertoniay == max(rf_hypertoniay), "yes", "no"
  )))
dat_xy$rf_diabetesy <-
  with(dat_xy, factor(ifelse(
    rf_diabetesy == max(rf_diabetesy), "yes", "no"
  )))
dat_xy$rf_hypercholesterolemiay <- with(
  dat_xy, factor(ifelse(
    rf_hypercholesterolemiay == max(rf_hypercholesterolemiay), "yes", "no")
  )
)
dat_xy$rf_smokery <-
  with(dat_xy, factor(ifelse(rf_smokery == max(rf_smokery), "yes", "no")))
dat_xy$rf_atrial_fibrillationy <- with(
  dat_xy, factor(ifelse(rf_atrial_fibrillationy == max(rf_atrial_fibrillationy),
    "yes", "no"))
)
...
pl_rf_hypercholesterolemiay <-
  vis_bin("rf_hypercholesterolemiay", "Hypercholesterolemia")
pl_rf_smokery <- vis_bin("rf_smokery", "Smoking")
pl_rf_atrial_fibrillationy <-
  vis_bin("rf_atrial_fibrillationy", "Atrial fibrillation")
pl_rf_chdy <- vis_bin("rf_chdy", "CHD")
pl_mrs_before <-
  ggplot(dat_xy, aes(x = dat_xy[, "mrs_before"], fill = mrs3)) +
  geom_bar(width = 0.8, position = position_stack(reverse = TRUE)) +
  theme_bw() +
...
pl_nihss_baseline <-
  ggplot(dat_xy_raw, aes(x = nihss, fill = mrs3)) +
  geom_bar(width = 0.8, position = position_stack(reverse = TRUE)) +
...
ggsave(file.path(out_dir, "figureB1.pdf"),
       width = 13,
       height = 8)
