...
pl_met <- function(spl_met,
                   ...
                   mod_labs = c(
                     "silscs" = expression(paste("SI-", CS[B], "-", LS[x])),
                   ...
                   ticks = TRUE,
                   col_ens = c(
                     "avg" = qualitative_hcl(n = 3, l = 80)[1],
  ...
  ggplot(
    ...
    (if ("cint" %in% metrics)
      geom_hline(
        data = data.frame(metric = "cint",
                          val = 0),
        aes(yintercept = val),
        alpha = 0.2
      )) +
    (if ("cslope" %in% metrics)
      geom_hline(
        data = data.frame(metric = "cslope",
                          val = 1),
        aes(yintercept = val),
        alpha = 0.2
      )) +
    ...
    geom_errorbar(
      data = ci %>% {
        if (!rel)
          ...
          filter(., metric %in% paste0("d", metrics)) %>%
    ...
    scale_color_manual(
      ...
      labels = if (!is.null(ref))
        ens_labs[names(ens_labs) %in% 
                   levels(spl_met$method)[levels(spl_met$method) != 
                                            unique(spl_met[spl_met$mod %in% ref, "method"])]]
      else
        ens_labs_noref,
      values = col_ens,
...
pl_or <- function(indiv,
                  ...
                  order_vars = TRUE) {
  uw <-
    indiv %>% group_by(var, mod, spl) %>% summarise(
      avg = mean(lor),
  ...
  uw_pooled <-
    indiv %>% filter(mod != "sils") %>% 
    group_by(var, mod, spl) %>% summarise(bs = bs(lor, ret_all = T)) %>%
  ...
  if (is.null(ci_sils)) {
    uw_pooled_sils <-
      indiv %>% filter(mod == "sils") %>% 
      group_by(var, mod) %>% summarise(bs = bs(lor, ret_all = T)) %>%
      group_by(var, mod) %>% summarise(lwr = quantile(bs, 0.025, na.rm = T),
                                       upr = quantile(bs, 0.975, na.rm = T))
  ...
  uw_pooled_avg <-
    uw %>% group_by(var, mod) %>% summarise(avg = mean(avg))
  uw_pooled <-
    left_join(uw_pooled, uw_pooled_avg, by = c("var", "mod"))
  ...
  if (!weighted) {
    ...
    w_pooled <-
      indiv %>% filter(mod != "sils") %>% group_by(var, mod, spl) %>% 
      summarise(bs = bs(lor, weights = w, ret_all = T)) %>%
      group_by(var, mod) %>% summarise(lwr = quantile(bs, 0.025, na.rm = T),
                                       upr = quantile(bs, 0.975, na.rm = T))
    if (is.null(ci_sils)) {
    ...
    w_pooled_avg <-
      w %>% group_by(var, mod) %>% summarise(avg = mean(avg))
    w_pooled <-
      left_join(w_pooled, w_pooled_avg, by = c("var", "mod"))
  ...
  if (order_vars) {
    vord <-
      indiv %>% group_by(var) %>% summarise(avg = mean(lor)) %>% arrange(avg)
  ...
  if (!pooled_only) {
    ggplot(spl_avg, aes(
      ...
      (if (lbetvar)
        geom_vline(
          xintercept = seq(1.5, length(unique(indiv$var)), by = 1),
          alpha = 0.05
        )) +
      (if (!y.text)
    ...
    ggplot(avg, aes(x = midpoint, y = prop, color = method)) +
      (if (!strokep) {
        (if (weighted) {
          facet_grid(weights ~ mod,
                     labeller = labeller(
                       mod = as_labeller(facet_labs, label_parsed),
                       .rows = label_both
                     ))
        } else {
          facet_grid( ~ mod, 
      ...
      (
        if (!is.null(avg_ref))
          geom_errorbar(
            data = avg_ref,
            aes(x = midpoint, ymin = lwr, ymax = upr),
            color = col_ref,
            size = ebarsize,
            inherit.aes = FALSE,
            width = 0.06
...
boot_ci <- function(cdf_all,
  ...
  if (!(mod %in% c("si", "sils"))) {
    ...
    if (!weighted & !avg) {
      ...
      ens <-
        lapply(lapply(lys_cdf_all, get_ensemble, type = meth), list)
    } else if (weighted & !avg) {
      # weighted ens
      ens <- lapply(get_weighted_ens(
        lys_cdf_all = lys_cdf_all,
        lys_weigths = w,
        type = meth
      ),
      list)
...
calc_calibration <- function(cdf_all,
                             y_true_all,
                             mod,
                             meth,
                             K,
  ...
  if (!(mod %in% c("si", "sils"))) {
    ...
    # no nested list
    lys_cdf_all <-
      lapply(lys_cdf_all, function(x)
        do.call(rbind, x))
    cal <-
      comb_ens_cal(
        lys_cdf_ens = lys_cdf_all,
        y_true_all = y_true_all,
        cumulative = TRUE,
        emp = TRUE,
        custom_cuts_fun = cuts_fun
...
cal_ens <-
  function(cdf,
           ...
           pool = TRUE,
           emp = FALSE,
           custom_cuts_fun = function(x)
             quantile(x, c(0.5, 0.9, 0.99, 0.999))) {
    cdf <- as.matrix(cdf)
    K <- ncol(cdf)
    ...
    for (cl in start:end) {
      if (cumulative) {
        yt <-
          factor(apply(y_true, 1, function(x)
            sum(x[start:cl])), levels = c(0, 1)) # levels = c(1, 0)) P(Y<y_k)
        yp <- 1 - apply(pdf, 1, function(x)
          sum(x[start:cl]))
      } else {
      ...
      if (K == 2) {
        lys_cal <-
          list(calibration(true ~ pred, data = df, cuts = cuts)$data)
      } else {
        lys_cal[[cl]] <-
          calibration(true ~ pred, data = df, cuts = cuts)$data
      }
    }
...
comb_avg_cal <-
  function(lys_cdf_all,
           ...
           custom_cuts_fun = function(x)
             quantile(x, c(0.5, 0.9, 0.99, 0.999))) {
    lys <- mapply(
      function(lys_cdf, y_true, cuts, weights, emp) {
        cal_splavg(
          lys_cdf = lys_cdf,
          y_true = y_true,
          cumulative = cumulative,
...
avg_across_spl <- function(lys_splitted) {
  lys_avg <- lapply(lys_splitted, function(x) {
    ...
    avg <- apply(simplify2array(tmp), 1:2, mean)
    data.frame(
      bin = if (length(unique(x$bin)) != length(avg[, "prop"]))
        NA
      else
        unique(x$bin),
      prop = avg[, "prop"],
...
extract_w <-
  function(w_all,
           meth = c("linear", "log-linear", "trafo"),
           ens = 5) {
    meth <- match.arg(meth)
    w_all %>%
      filter(method  == meth) %>%
